under_twenty = {
    '0': 'zero',
    '1': 'one',
    '2': 'two',
    '3': 'three',
    '4': 'four',
    '5': 'five',
    '6': 'six',
    '7': 'seven',
    '8': 'eight',
    '9': 'nine',
    '10': 'ten',
    '11': 'eleven',
    '12': 'twelve',
    '13': 'thirteen',
    '14': 'fourteen',
    '15': 'fifteen',
    '16': 'sixteen',
    '17': 'seventeen',
    '18': 'eighteen',
    '19': 'nineteen'
}
tens_place = {
    '2': 'twenty',
    '3': 'thirty',
    '4': 'forty',
    '5': 'fifty',
    '6': 'sixty',
    '7': 'seventy',
    '8': 'eighty',
    '9': 'ninety'
}
hundreds_place = {
    '1': 'one hundred',
    '2': 'two hundred',
    '3': 'three hundred',
    '4': 'four hundred',
    '5': 'five hundred',
    '6': 'six hundred',
    '7': 'seven hundred',
    '8': 'eight hundred',
    '9': 'nine hundred'
}
while True:
    user_num = int(input('What is your number: '))
    tens_digit = str(user_num//10)
    ones_digit = str(user_num%10)
    hundreds_digit = str(user_num//100)
    if user_num < 20:
        user_string = str(user_num)
        if user_string in under_twenty:
            print(under_twenty[user_string])
    elif user_num < 100:
        if ones_digit != '0':
            print(f'{tens_place[tens_digit]}-{under_twenty[ones_digit]}')
        else:
            print(tens_place[tens_digit])
    elif user_num < 1000:
        if ones_digit == '0' or tens_digit == '0':
            if ones_digit == '0' and tens_digit == '0':
                print(hundreds_place[hundreds_digit])
            elif ones_digit == '0':
                print(f'{hundreds_place[hundreds_digit]} and {tens_place[tens_digit]}')
            else:
                print(f'{hundreds_place[hundreds_digit]} and {tens_place[tens_digit]}-{under_twenty[ones_digit]}')
            # print(f'{tens_place[tens_digit]}-{under_twenty[ones_digit]}')
        else:
            print(f'{hundreds_place[hundreds_digit]} and {tens_place[tens_digit]}-{under_twenty[ones_digit]}')
    
    else:
        print('Not valid')
    repeat = input('Would you like to convert something else? y/n: ')
    if repeat == 'n':
        print('Bye Bye')
        break   